...
static uint8_t fevent_control_motor(uint8_t event);
...
sEvent_struct       sEventAppFeed[]=
{
  ...
  {_EVENT_FEED_SHRIMP,           0, 5, 20,       fevent_feed_shrimp},
  {_EVENT_CONTROL_MOTOR,         1, 5, 0,        fevent_control_motor},
...
static uint8_t fevent_feed_shrimp(uint8_t event)
{
    ...
    static uint32_t Time_WaitUltrasonic = 0;
    ...
    if(sParamFeed.Power == _ON_FEED)
    {
        ...
        if(sParamFeed.State==1 && sParamFeed.Time>=sParamFeed.TimeRun*60 )
        {
...
//                Off_MotorLevel();
                ...
                Time_WaitUltrasonic = HAL_GetTick();
            }
            else
            {
                if(State_Ultrasonic == 1)
                {
                    if(HAL_GetTick() - Time_Ultrasonic_On > 5000)
                    {
                        State_Ultrasonic = 0;
                        Time_Ultrasonic_Off = HAL_GetTick();
                        OnOff_Relay(RELAY_3, OFF_RELAY);
                ...
                {
                    if(HAL_GetTick() - Time_Ultrasonic_Off > 5000)
                    {
                        State_Ultrasonic = 1;
                        Time_Ultrasonic_On = HAL_GetTick();
                        OnOff_Relay(RELAY_3, ON_RELAY);
        ...
        if(sParamFeed.State==3 && HAL_GetTick() - Time_WaitUltrasonic > 0)
        {
        ...
        if(sParamFeed.State==4 && sParamFeed.Time>=sParamFeed.TimeFree*60)
        {
...
//    static uint32_t Time_Ultrasonic_Off = 0;
    ...
    static uint32_t GetTick_WaitChange = 0;
...
//    static uint8_t State_Ultrasonic = 0;
    ...
    if(sParamFeed.Power == _ON_FEED)
    {
        ...
        if(sParamFeed.State==1 && sParamFeed.Time>=sParamFeed.TimeRun*60 )
        {
            ...
            sParamFeed.State++;
            sParamFeed.Time=0;
            GetTick_WaitChange = HAL_GetTick();
        }
        
        if(sParamFeed.State==2 && HAL_GetTick() - GetTick_WaitChange > sParamFeed.TimeCycle)
        {
            OnOff_Relay(RELAY_2, OFF_RELAY);
            sParamFeed.State++;
            sParamFeed.Time=0;
        ...
        if(sParamFeed.State==3 && sParamFeed.Time>=sParamFeed.TimeFree*60)
        {
            OnOff_Relay(RELAY_2, ON_RELAY);
            sParamFeed.State++;
            sParamFeed.Time=0;
        ...
        if(sParamFeed.State==4 && HAL_GetTick() - GetTick_WaitChange > sParamFeed.TimeCycle)
        {
            OnOff_Relay(RELAY_3, OFF_RELAY);
            sParamFeed.Time=0;
            sParamFeed.State=0;
