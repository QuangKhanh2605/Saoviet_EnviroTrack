...
uint8_t aState_Bom[17]          = {"Bom 1           "};
uint8_t aState_Xa[17]           = {"Bom 2           "};
uint8_t aState_Ultrasonic[17]   = {"DeGas           "};

/*===== Object LCD =============== MAIN Screen 1 =================*/
LCD_Char_Display                    oLCD_C_Main_1[]=
{
  {_LCD_C_MAIN_1_RUNNING,       0,0,0,          (uint8_t*)"Running "},
  {_LCD_C_MAIN_1_LEVEL,         0,1,0,          (uint8_t*)"Bom Nuoc        "},
  {_LCD_C_MAIN_1_RUN,           0,1,0,          (uint8_t*)"Bom 1:      Phut"},
  {_LCD_C_MAIN_1_FREE,          0,1,0,          (uint8_t*)"Bom 2:      Phut"},
  {_LCD_C_MAIN_1_CYCLE,         0,1,0,          (uint8_t*)"Delay:        ms"}
};
...
{
  {_LCD_C_MAIN_2_SETUP,         0,0,0,          (uint8_t*)"Cai dat "},
  {_LCD_C_MAIN_2_RUN,           0,1,0,          (uint8_t*)"Bom 1:      Phut"},
  {_LCD_C_MAIN_2_FREE,          0,1,0,          (uint8_t*)"Bom 2:      Phut"},
  {_LCD_C_MAIN_2_CYCLE,         0,1,0,          (uint8_t*)"Delay:        ms"},
};
...
void        DLCD_Main_1(void)
{
    for(uint8_t i = 0; i < _LCD_C_MAIN_1_END; i++)
    {
        if(oLCD_C_Main_1[i].Status == 1)
        {
            ...
            CLCD_WriteString(&LCD, (char*)oLCD_C_Main_1[i].Object);
...
void        DLCD_Main_2(void)
{
    for(uint8_t i = 0; i < _LCD_C_MAIN_2_END; i++)
    {
        if(oLCD_C_Main_2[i].Status == 1)
        {
            ...
            CLCD_WriteString(&LCD, (char*)oLCD_C_Main_2[i].Object);
...
void Main_1_Level_Alter(void)
{
    ...
    if(stamp != sParamFeed.State)
    {
        oLCD_C_Main_1[_LCD_C_MAIN_1_LEVEL].Status = 1;
        
        switch(sParamFeed.State)
        {
            case 0:
            case 1:
            case 2:
              oLCD_C_Main_1[_LCD_C_MAIN_1_LEVEL].Object = aState_Bom;
              break;
              
            case 5:
            case 6:
              oLCD_C_Main_1[_LCD_C_MAIN_1_LEVEL].Object = aState_Ultrasonic;
              break;
              
            case 3:
            case 4:
              oLCD_C_Main_1[_LCD_C_MAIN_1_LEVEL].Object = aState_Xa;
              break;
          
            default:
              break;
        }
        stamp = sParamFeed.State;
    }
